...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="keyExtnName"/>
	<xsl:param name="prodKeyName" />
	<xsl:param name="testKeyName" />
			...
			<xsl:element name="configurationTypes">
				<xsl:element name="ManagedSshKeyType">
					...
					<xsl:element name="extensions">
						<xsl:element name="ManagedSshKeyTypeExtn">
							<xsl:attribute name="extensionName">
								<xsl:value-of select="$keyExtnName"/>
							</xsl:attribute>
							
							<xsl:attribute name="extensionValue">
								<xsl:value-of select="$prodKeyName" />
							</xsl:attribute>
						</xsl:element>
					</xsl:element>
				</xsl:element>
				...
				<xsl:element name="ManagedSshKeyType">
					...
					<xsl:element name="extensions">
						<xsl:element name="ManagedSshKeyTypeExtn">
							...
							<xsl:attribute name="extensionValue">
								<xsl:value-of select="$testKeyName" />
							</xsl:attribute>
						</xsl:element>
					</xsl:element>
				</xsl:element>
