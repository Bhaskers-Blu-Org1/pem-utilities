 ...
 * Public part of UserIdentityKey would be checked to PR as Authorized user Key.
 */
public class UserIdentityKeyImportProcessor extends PrConfigurationProcessor {
	...
	public String getKeyData(boolean getFromProductionSfg) throws ApiInvocationException, ImportException, ParserConfigurationException, SAXException, IOException {
		Document sshRemoteProfileData = getFromProductionSfg ? configInfo.getProdRemoteProfileDoc()
				: configInfo.getTestRemoteProfileDoc();
		String userIdentityKey = SftpResourceHelper.getAttributeValueByTagName(sshRemoteProfileData, "SSHRemoteProfile",
				"userIdentityKey");
		String userIdentityKeyData = SftpResourceHelper.getResourceFromSFG(getConfig(), getFromProductionSfg, GET_MANAGED_USERIDENTITY_KEY_DATA_URL, userIdentityKey)
				.getResponse();
		//get public part of the key
		return SftpResourceHelper.getAttributeValueByTagName(userIdentityKeyData, "SSHUserIdentityKey", "publicKeyData");
	}
	...
	protected String createProfileConfiguration(String resourceType, String subResourceType, String resourceName)
		...
		try {
			String requestXML = SftpResourceHelper.createXml(new ProfileConfigurationXSLT().createProfileConfiguration(
					...
					getPartnerInfo().getPemPartnerKey(), getPartnerInfo().getSponsorDivisionKey(), getPartnerInfo().getAuthUserKeySGProfileConfig()));
	...
	protected String createConfigResource(String profileConfigKey) throws ImportException {
		try {
			String requestXml = SftpResourceHelper.createXml(UserIdentityKeysXSLT.createManagedSshkeys(getConfig(),
					profileConfigKey, getKeyData(true), getKeyData(false), getKeyType(), getKeyName(true), getKeyName(false)));
			return SftpResourceHelper.callCreateApi(getConfig(), requestXml, getConfig().buildPRUrl(CREATE_MANAGED_SSH_KEY_API_URL), getResourceKeyAttributeName());
	...
	protected String getProfileConfigurationURL(String resourceType, String subResourceType, String resourceName) {
		return getConfig().buildPRUrl(CREATE_PROFILECONFIGURATION_API_URL)
				+ String.format("?serverType=%s&resourceType=%s&subResourceType=%s&name=%s",
						Constants.SERVER_TYPE_DG, resourceType, subResourceType, resourceName);
	}
	...
	private void validateSGProfileConfiguration(String sgProfileConfigId, String resourceType, String subResourceType)
		...
		if (sgProfileConfigId == null) {
			throw new ImportException("AuthorizedUserKey Profile Configuration for " + getSubResourceType() + " is not specified." );
		}
		try {
			...
			String profileResourceType = SftpResourceHelper.getAttributeValueByTagName(apiResponse, "resourceType",
					"code");
			String profileSubResourceType = SftpResourceHelper.getAttributeValueByTagName(apiResponse, "subResourceType",
					"code");
			String profileServerType = SftpResourceHelper.getAttributeValueByTagName(apiResponse, "serverType",
					"code");
			if (!Constants.SERVER_TYPE_SG.equals(profileServerType) || !resourceType.equals(profileResourceType)
				...
				throw new ImportException("AuthorizedUserKey Profile Configuration for " + getSubResourceType() + " must be of type SG.");
			}
